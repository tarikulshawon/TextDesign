...
class Store: NSObject {
    ...
    //var allAssest = [ImageInformation]()
    ...
    func verifyReciept () {
        ...
        let appleValidator = AppleReceiptValidator(service: .production, sharedSecret: "06c7359b784b46bebd3d7ec6a66ee0bf")
        SwiftyStoreKit.verifyReceipt(using: appleValidator) { result in
            switch result {
            case .success(let receipt):
                let productIds = Set([ PoohWisdomProducts.yearlySub,
                                       PoohWisdomProducts.weeklySub,
                                       PoohWisdomProducts.monthlySub])
